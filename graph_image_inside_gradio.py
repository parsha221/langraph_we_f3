...
def gradio_interface(file1, file2, file3, user_question):
    ...
    if len(files) > 0:
        ...
        try:
            ...
            graph_image_path = graph.draw_graph()
            return output_state["response"], output_state["matched_file"], graph_image_path
...
iface = gr.Interface(
    fn=gradio_interface,
    inputs=[
        gr.File(type="filepath", label="Upload PDF file 1"),
        gr.File(type="filepath", label="Upload PDF file 2"),
        gr.File(type="filepath", label="Upload PDF file 3"),
        gr.Textbox(label="Type Your Question Here"),
    ],
    outputs=["text", "text", gr.Image()],
    title="Chatbot with PDF Support",
    description="Upload one or more PDF files and ask questions about their content. The processing graph will be displayed.",
)
